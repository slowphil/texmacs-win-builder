...
@"%cwd%"\usr\bin\pacman -S --needed --force --noconfirm ^
	...
	mingw-w64-x86_64-git mingw-w64-x86_64-toolchain ^
	mingw-w64-x86_64-curl mingw-w64-x86_64-expat ^
	mingw-w64-x86_64-openssl mingw-w64-x86_64-tcl ^
	mingw-w64-x86_64-pcre mingw-w64-x86_64-connect ^
	git-flow
...
@IF NOT DEFINED JENKINS_URL @(
	...
	@ECHO.
	@ECHO Installing the 'Git SDK 64-bit' shortcut on the Desktop
	@bash --login -c 'SHORTCUT="$HOME/Desktop/Git SDK 64-bit.lnk"; test -f "$SHORTCUT" ^|^| create-shortcut.exe --icon-file /msys2.ico --work-dir / /git-bash.exe "$SHORTCUT"'

	@REM now clone the Git sources, build it, and start an interactive shell
	@bash --login -c "mkdir -p /usr/src && cd /usr/src && for project in MINGW-packages MSYS2-packages build-extra; do test ! -d $project && mkdir -p $project && (cd $project && git init && git config core.autocrlf false && git remote add origin https://github.com/git-for-windows/$project); done; if test ! -d git; then git clone -b master -c core.autocrlf=false https://github.com/git-for-windows/git; fi && cd git && make install"

	@IF ERRORLEVEL 1 PAUSE

	@start mintty -i /msys2.ico -t "Git SDK 64-bit" bash --login -i
)
